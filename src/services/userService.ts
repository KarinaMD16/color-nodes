...
export async function getLeaderboard(roomCode: string): Promise<UserRoomRankDto[]> {
  const { data } = await colorNodesAPI.get<UserRoomRankDto[]>(`/Room/${roomCode}/leaderboard`);
  return data;
}
